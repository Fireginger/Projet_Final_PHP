...
//--------------------------------- TRAITEMENTS PHP ---------------------------------//
if(isset($_GET['id']))  { $resultat = executeRequete("SELECT article.id, article.name, article.catégorie, article.image_link, article.description, article.price, stock.quantity FROM article INNER JOIN stock ON stock.article_id = article.id WHERE article.id = '$_GET[id]'"); }
